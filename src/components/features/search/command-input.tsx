 ...
 * 🍒 Cherry - 命令输入组件
 *
 * 终端风格的命令输入框，支持命令历史、自动补全和搜索建议。
 * 是用户与应用交互的核心入口。
 *
 * @file src/components/features/search/command-input.tsx
 *
 * @description
 * 功能：
 * - 命令执行：help, ls, go, g, clear
 * - 搜索过滤：实时过滤链接
 * - 命令历史：上下箭头浏览
 * - 自动补全：Tab 键循环建议
 * - 搜索建议：下拉显示匹配项
...
export function CommandInput({
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 mb-2 relative">
      ...
      {/* 命令提示 */}
      <div 
        className="mt-2 text-xs text-[var(--cherry-muted)] font-code text-center"
        suppressHydrationWarning
      >
        <span className="text-[var(--cherry-amber)]" suppressHydrationWarning>{t('command.commands')}</span>{' '}
        ...
        {' '}|{' '}
        <span className="text-[var(--cherry-amber)]" suppressHydrationWarning>{t('command.tips')}</span>{' '}
